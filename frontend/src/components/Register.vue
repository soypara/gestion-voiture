  ...
  <div id="register-page">
    <div class="register-box">
      ...
      <p class="login-link">Déjà inscrit ? 
        <span @click="$router.push('/login')">Connectez-vous</span>
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async handleRegister() {
      try {
        // 🔥 IMPORTANT : backend doit être le nom du service Docker
        ...
        if (res.ok) {
          ...
          alert(data.email ? data.email.join(" ") : "Erreur inscription");
        }
      } catch (err) {
        console.error(err);
